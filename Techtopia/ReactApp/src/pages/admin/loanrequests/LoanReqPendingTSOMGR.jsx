...
import { Typography, Button, Dialog, DialogContent, DialogActions, DialogTitle, Grid } from '@mui/material'
...
function RenderButton(props) { // Button to view loan request details
  ...
  const handleClose = () => {
    setOpen(false);
    console.log('Items:', reservedItems);
    setReservedItems([]);
  };

  const handleApprove = () => {
    const loanRequestId = element.loanRequestId
    const assetIds = reservedItems.map(item => item[0].itemId)
    const reqBody = {
    ...
    <>
      ...
      >
        ...
        <DialogActions style={{ justifyContent: "space-between" }}>
          ...
          <Button varient='contained' sx={{ ...otherButtonStyles }} onClick={handleApprove}>
            Approve Request
          </Button>
        </DialogActions>
      </Dialog >
...
function LoanReqPendingTSOMGR() {
  ...
  const fetchData = async () => {
    ...
    const [loanRes] = await Promise.all([
      http.get(`/LoanRequestByLoanStatus?loanRequestStatus=1`),
    ]);
...
export default LoanReqPendingTSOMGR