...
const initialState = {
    ticketId: localStorage.getItem("ticket_id") ? localStorage.getItem("ticket_id") : -1,
    userRole: localStorage.getItem("userRole") ? localStorage.getItem("userRole") : DEFAULT_USER_ROLE,
    visitorJWT: localStorage.getItem("accessToken") ? localStorage.getItem("accessToken") : null,
    adminJWT: localStorage.getItem("adminAccessToken") ? localStorage.getItem("adminAccessToken") : null,
}
...
export const userSlice = createSlice({
    ...
    reducers: {
        ...
        boothHelperLogin: (state, action) => {
            console.log(`boothHelperLogin Login Payload - ${JSON.stringify(action.payload)}`);
        ...
        adminLogin: (state, action) => {
            console.log(`adminLogin Login Payload - ${JSON.stringify(action.payload)}`);
...
export const { boothadminlogout, updateUserRole, visitorLogin, boothHelperLogin, adminLogin } = userSlice.actions
